"""M10 Chromium diagnostic with inlined source to bypass environment-local-URL block.
...
with sync_playwright() as p:
    ...
    try:
        for kind,kw in [('desktop',dict(viewport=dict(width=1280,height=800),locale='en-US')),
            ...
            assert 'M10' in page.title()
            assert page.locator('#menu-items button').count()==4
            # M10 accessibility: browser Tab focus and Enter use the focused item,
            ...
            output=os.environ.get('M10_SCREENSHOT_DIR')
            if output:
                Path(output).mkdir(parents=True,exist_ok=True)
                page.screenshot(path=str(Path(output)/f'browser-m10-{kind}.png'))
            print(f'BROWSER M10 {kind}: PASS (inline diagnostic; native text import/export; persistence simulated; malformed file rejected; no page scroll)')
            page.close()
