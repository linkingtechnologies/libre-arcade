...
parts=[]
for name in ['asteroids.js','particles.js','saucers.js','combat.js','core.js','render.js','storage.js','menu.js','native-files.js','input.js','frame-delta.js','app.js']:
...
with sync_playwright() as p:
    ...
    try:
        for kind,kw in [('desktop',dict(viewport=dict(width=1280,height=800),locale='en-US')),
            ...
            output=os.environ.get('M9_SCREENSHOT_DIR')
            if output:
                Path(output).mkdir(parents=True,exist_ok=True)
                page.screenshot(path=str(Path(output)/f'browser-m9-{kind}.png'))
            print(f'BROWSER M9 {kind}: PASS (inline diagnostic; native text import/export; persistence simulated; malformed file rejected; no page scroll)')
            page.close()
