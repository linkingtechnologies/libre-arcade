"""M11 Chromium diagnostic with inlined source to bypass environment-local-URL block.
...
storage_stub="""Object.defineProperty(window,'localStorage',{configurable:true,value:{getItem:k=>window.__store.get(k)??null,setItem:(k,v)=>window.__store.set(k,v),removeItem:k=>window.__store.delete(k)}});"""
with sync_playwright() as p:
    ...
    try:
        for kind,kw in [('desktop',dict(viewport=dict(width=1280,height=800),locale='en-US')),
            ...
            assert page.title()=='Asteroids Infinity | Libre Arcade'
            assert page.locator('#menu-items button').count()==4
            # M11 accessibility: browser Tab focus and Enter use the focused item,
            # not the (potentially different) arrow-selected menu index.
            page.locator('#menu-items button').first.focus()
            page.keyboard.press('Tab')
            assert page.evaluate('document.activeElement===document.querySelectorAll("#menu-items button")[1]'), 'Tab navigation blocked'
            page.keyboard.press('Enter')
            assert page.locator('.score-table tr').count()==10, 'Enter selected wrong item'
            page.keyboard.press('Escape')
            assert page.locator('#menu-items button').count()==4
            page.locator('#lang').focus()
            previous=page.evaluate('document.documentElement.lang')
            page.keyboard.press('Enter')
            assert page.evaluate('document.documentElement.lang')!=previous, 'Enter blocked header button'
            page.locator('#lang').click()  # restore baseline language for further checks
            ...
            assert page.locator('#menu-items button').first.inner_text().endswith('W')
            page.keyboard.press('Escape') # controls -> options
            page.evaluate("window.__store.set('another-application','keep')")
            # Cancel must preserve previously imported data.
            page.once('dialog',lambda d:d.dismiss())
            page.locator('.transfer-button').nth(2).click()
            assert 'TESTER' in page.evaluate("window.__store.get('libre-arcade.ai.highscores.m6')")
            # Explicit confirmation deletes only this game's saved keys.
            page.once('dialog',lambda d:d.accept())
            page.locator('.transfer-button').nth(2).click()
            assert page.evaluate("window.__store.get('another-application')")=='keep'
            assert not page.evaluate("window.__store.has('libre-arcade.ai.highscores.m6') || window.__store.has('libre-arcade.ai.bindings.m6')")
            page.keyboard.press('Escape') # options -> menu
            page.locator('#menu-items button').first.click() # PLAY
            assert page.locator('#menu-overlay').is_hidden(), 'Play failed to hide menu'
            page.locator('#pause').click()
            assert page.locator('#menu-overlay').is_visible() and page.locator('#menu-title').inner_text() in ['PAUSED','PAUSA'], 'Pause failed'
            page.locator('#menu-items button').first.click()  # Resume
            assert page.locator('#menu-overlay').is_hidden(), 'Resume failed'
            ...
            output=os.environ.get('M11_SCREENSHOT_DIR')
            if output:
                Path(output).mkdir(parents=True,exist_ok=True)
                page.screenshot(path=str(Path(output)/f'browser-m11-{kind}.png'))
            print(f'BROWSER M11 {kind}: PASS (inline diagnostic; native text import/export; persistence simulated; malformed file rejected; no page scroll)')
            page.close()
