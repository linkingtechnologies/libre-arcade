...
| [`asteroids-infinity/`](asteroids-infinity/) | [Asteroids Infinity](https://linkingtechnologies.github.io/libre-arcade/asteroids-infinity/public/index.html) | *Ported.* [Asteroids Infinity](https://sourceforge.net/projects/asteroidsinf/) 1.2 (Ben Whittaker, 2009, GPL-3.0-or-later) — a single 1012-line Python 2/Pygame script whose own headline feature was its elastic camera, loosely attached to a ship that never slows down, reimplemented in Canvas 2D. The original asteroid construction and drift code and the historical `controls.txt`/`highscores.txt` routines are executed as oracles; the source's own quirks are kept rather than corrected, including the degenerate random draw that gives every asteroid the same spin. The Vector Battle font and the original WAV effects have unresolved redistribution terms and are not included; the browser build synthesizes its own sounds. |
| [`oglbricks/`](oglbricks/) | [OGLBricks](https://linkingtechnologies.github.io/libre-arcade/oglbricks/public/index.html) | *Ported.* [OGLBricks](https://sourceforge.net/projects/oglbricks/) 0.2 (Alexey Markarov, 2013, MIT) — a falling-block game whose two unusual ideas are a field sized freely from 10 to 50 on each axis and a catalog of 27 source-defined pieces grouped by block count, any group of which can be switched off, reimplemented from C++/Qt/OpenGL in Canvas 2D. Every shape and each of its four turn states is checked against an extraction of the original `Shape.cpp`. The original program is a Windows binary and has never been run in this project, so the rules rest on source review; one native observation of the score disagrees with the source formula and is recorded as open. The Qt and Assimp libraries, the 3D models and the original audio are not redistributed; the browser build draws and synthesizes its own. |
...
| [Asteroids Infinity](https://sourceforge.net/projects/asteroidsinf/) 1.2 (2009) | Ben Whittaker | GPL-3.0-or-later | `asteroids-infinity/` — the "or later" grant confirmed in the header of the preserved Python source, where the SourceForge project page states only "GPLv3"; the original release ZIP, the Vector Battle font and the WAV effects are not redistributed (rights never established); the JS port itself is GPL-3.0-or-later |
| [OGLBricks](https://sourceforge.net/projects/oglbricks/) 0.2 (2013) | Alexey Markarov | MIT | `oglbricks/` — the MIT grant confirmed on the project page and in the preserved upstream notice, whose copyright line reads 2012; the four original archives are not redistributed and not kept here either (Qt 4 and Assimp libraries, 3D models with unestablished terms), only their SHA-256 and the upstream folders they came from, both re-checked; the piece geometry keeps its MIT attribution inside the GPL port, and the JS port itself is GPL-3.0-or-later |
