...
// contains whichever frame happens to be showing, never clipping past it. Reported live: touching
// an enemy now costs a life and
// reloads the level (see player.js's hurt()), and the ground-crawlers' full 64x64 tile-sized
// sensor (no override = ENEMY_SIZE default, still true for frog/saw below) made clearing them
// with a jump "fisicamente impossibile" — a running jump only rises above a 64px-tall box for a
// ~54px-wide slice of its ~90px total horizontal travel, well short of the ~122px (enemy+player
// hitbox width) needed to cross clean. The actual creature art for slime/snail/ladybug/worm/
// mouse/bee/fly/barnacle/fish sits well below the tile's top edge (their real silhouette leaves
// 3-40px of transparent headroom, checked directly on disk) — the oversized box was never a
// deliberate difficulty choice, just an unmeasured default nobody had reason to shrink back when
// contact was harmless. frog and saw both genuinely fill their whole tile (checked the same way,
// 0px margin on every frame) — frog is left at the full ENEMY_SIZE default (an upright frog
// reads as something you're meant to route around, not hop over, and nobody's reported it as
// unstompable), but saw got its own override below anyway: with the full-tile box (sensor top
// flush with the tile's own top edge, zero headroom), isStomp()'s own geometry math leaves only
// a ~28px-tall sliver of the full 64px tile — right at the very top — where a landing reads as a
// stomp instead of a hurt, reported live as functionally impossible ("se salto sopra questo
// nemico non lo uccido"). h:50 isn't measured off the art (see this comment's own "0px margin"
// note above — the circular sawblade art genuinely has none) — it's the same deliberate
// "err toward more forgiving than the art strictly implies" call trapcoin.js's own hitbox already
// makes, just enough headroom (14px) to make stomping reliably possible without making a
// spinning blade as forgiving as a soft ground-crawler.
// `behavior` (default "patrol" — see BEHAVIOR_PATROL etc. below) comes from each world manifest's
// own prose ("behavior" field in assets/worlds/*.json), NOT auto-derived from it — that text is
// free-form Italian description for humans, not a machine-readable enum. Previously ignored
// entirely: every kind fell through to the same generic left-right patrol regardless of what its
// manifest said, so saw ("oscilla o gira sul posto") and barnacle ("fisso, attacca quando ci si
// avvicina") both paced back and forth exactly like mouse's own explicitly-patrolling entry, and
// frog ("salta a intervalli regolari") walked with a smooth continuous slide instead of discrete
...
const KINDS = {
  ...
  // World2 (Bosco degli Scoiattoli).
  worm:     { rest: "enemy-worm-rest",     walkA: "enemy-worm-walk-a",     walkB: "enemy-worm-walk-b",     hitbox: { w: 64, h: 28 } },
  // frog: "salta a intervalli regolari" — see BEHAVIOR_JUMP / updateJump() below.
  ...
  // saw: "oscilla o gira sul posto" — stationary, not a patroller; the existing walkA/walkB frame
  ...
  // World4 (Miniera di Pietra).
  // barnacle: "fisso, attacca quando ci si avvicina" — fisso (fixed) is BEHAVIOR_STATIONARY
  // below; "attacca quando ci si avvicina" is already what touching its sensor does once it's no
  // longer chasing/patrolling toward the player (see PlayerEntity.onCollision's hurt() call) — a
  // wider proximity-detection radius with its own aggro range is a bigger behavioral feature (it'd
  // need this entity to query the player's live position, which nothing here does today), out of
  // scope for fixing "stuck patrolling like every other enemy" specifically.
  ...
  // reported live ("non riesco ad uccidere il serpente saltandoci sopra" — this elongated,
  // lunging-attack creature is the closest match in the roster, though no literal "snake" kind
  // exists): 3px of real headroom is, in practice, the same "fisicamente impossibile" stomp window
  // saw had at 0px — err toward more forgiving (14px) than the art's own worst-case frame implies.
...
// jump ("a intervalli regolari"), not just a faster walk animation. JUMP_SPEED is well above
...
// World2's own manifest describes bee/fly as flying ("vola in pattern semplice"/"vola erratico"),
// not ground-crawling like worm/snail/slime/etc — but compile-level.py places every enemy object
// the same way (bottom-flush with the ground, same convention as GoalEntity/CoinEntity), so
// without this a bee/fly spawned exactly like a snail reads as sitting half-buried in the ground
// tile — reported live ("vedo un'ape nel terreno"). Kept as a simple static hover height, not an
...
class EnemyEntity extends me.Collectable {
  ...
  onResetEvent(x, y, settings = {}) {
    ...
    // had — reported live as an enemy sometimes standing correctly on the ground and sometimes
    // appearing sunk into it, specifically after leaving and re-entering a level (exactly when a
    // pooled instance gets reused instead of freshly constructed): "andando avanti, a volte il
    // nemico è sul terreno, a volte una posizione sotto... se rientro nel gioco, la volta
    // successiva può apparire nel terreno". Re-deriving kind/spawnY/image/minX/maxX here from
    ...
    // PREVIOUS life had, only the position/sprite/kind label were actually corrected. Reported
    // live as a stomp not registering on a worm ("se salto sul verme come nemico non lo uccido")
    // — a pool slot last constructed as a taller kind (or the full-tile default) reused for a
    // worm keeps that stale, differently-shaped sensor, so isStomp()'s geometry no longer matches
