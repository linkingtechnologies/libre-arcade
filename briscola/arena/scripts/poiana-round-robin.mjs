...
import { poianaArenaPlayers as arenaPlayers } from "./lib/players.mjs";
...
const outputBase = resolveProjectPath(process.argv[4] ?? "arena/results/latest-poiana-round-robin");
...
const totals = Object.fromEntries(
  arenaPlayers.map((player) => [player.id, { name: player.name, wins: 0, points: 0, games: 0 }])
);
const headToHead = [];
let totalDraws = 0;
let totalGames = 0;

for (let i = 0; i < arenaPlayers.length; i += 1) {
  for (let j = i + 1; j < arenaPlayers.length; j += 1) {
    const a = arenaPlayers[i];
    const b = arenaPlayers[j];
    const match = { a: a.id, b: b.id, nameA: a.name, nameB: b.name, games: seedPairs * 2, winsA: 0, winsB: 0, draws: 0, pointsA: 0, pointsB: 0 };
    ...
    for (let seed = seedStart; seed < seedStart + seedPairs; seed += 1) {
      ...
      for (const game of games) {
        ...
        match.pointsA += aPoints; match.pointsB += bPoints;
        totals[a.id].points += aPoints; totals[b.id].points += bPoints;
        totals[a.id].games += 1; totals[b.id].games += 1; totalGames += 1;
        if (game.result.draw) { match.draws += 1; totalDraws += 1; }
        else {
          const winnerId = game.result.winner === 0 ? game.first.id : game.second.id;
          if (winnerId === a.id) { match.winsA += 1; totals[a.id].wins += 1; }
          else { match.winsB += 1; totals[b.id].wins += 1; }
        }
      }
    }
...
const report = {
  format: "briscolab-arena-round-robin-v1",
  group: "poiana",
  seedStart,
  seedPairs,
  totalGames,
